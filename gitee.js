...
async function giteeUpdate() {
    ...
    // 2. 填入账号
    await accountElements[0].type('773515798@qq.com')
    // 3. 选中密码控件
    let pwdElements = await page.$x('//*[@id="user_password"]')
    // 4. 填入密码
    await pwdElements[0].type('lj131452401')
    // 5. 点击登录
    let loginButtons = await page.$x('//*[@id="new_user"]/div/div/div/div[4]/input')
    await loginButtons[0].click()
    // 6. 等待登录成功
    await page.waitFor(1000)
    await page.goto('https://gitee.com/kubPang/sourceFile/pages'); // 比如： https://gitee.com/rong0624/blog/pages
